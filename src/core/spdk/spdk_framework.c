 ...
 *
 * SPDK 25.09 and newer accept page-aligned external regions. Existing
 * native slab alignment is sufficient and needs no SPDK-specific padding.
 */
...
#if SPDK_VERSION_MAJOR < 25 || (SPDK_VERSION_MAJOR == 25 && SPDK_VERSION_MINOR < 9)
#error "libevpl requires SPDK 25.09 or newer"
#endif /* if SPDK_VERSION_MAJOR < 25 || (SPDK_VERSION_MAJOR == 25 && SPDK_VERSION_MINOR < 9) */
    ...
    void *thread_private)
{
    ...
    /* Host calls evpl_cleanup before spdk_env_fini. A later atexit cleanup
     * is idempotent and will not unregister these mappings again. */

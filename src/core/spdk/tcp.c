 ...
 *
 *   - a dedicated SPDK poller, independent of the native poll_mode setting;
 ...
 *
 * The mini-reactor tests exercise both polling and interrupt operation.
 */
...
static int
evpl_spdk_sock_poll_cb(void *arg)
{
    struct evpl_spdk_thread *ctx = arg;

    return evpl_spdk_sock_sweep(ctx->evpl, ctx) ? SPDK_POLLER_BUSY : SPDK_POLLER_IDLE;
} /* evpl_spdk_sock_poll_cb */

static void
evpl_spdk_sock_poll_mode(
...
evpl_spdk_sock_progress_inc(struct evpl_spdk_thread *ctx)
{
    if (ctx->progress++ == 0 && ctx->intr_registered &&
        ...
        evpl_add_oneshot_timer(ctx->evpl, &ctx->progress_timer,
...
evpl_spdk_sock_thread(struct evpl *evpl)
{
    ...
    ctx->poll = spdk_poller_register(evpl_spdk_sock_poll_cb, ctx, 0);
    evpl_spdk_sock_abort_if(!ctx->poll, "socket poller registration failed");
    ...
    if (spdk_interrupt_mode_is_enabled()) {
        ...
        spdk_poller_register_interrupt(ctx->poll, evpl_spdk_sock_poll_mode, ctx);
    }
    ...
    struct spdk_sock       *sock)
{
    ...
    for (iter = 0; iter < 8; iter++) {
        ...
        if (s->recv1.length >= res) {
            ...
            got += res;
            break; /* second receive vector was untouched: this is a short read */
        } else {
        ...
        /* Both staging vectors were consumed. Make the transition explicit
         * before the next read replenishes them; no reference remains owned
         * by either staging slot after a full append. */
        s->recv1.length = 0;
        s->recv2.length = 0;
    }
    ...
    struct evpl_timer *timer)
{
    ...
    if (ctx->progress && !ctx->progress_timer_armed) {
        evpl_add_oneshot_timer(evpl, &ctx->progress_timer,
                               evpl_spdk_sock_progress_timer_cb,
                               EVPL_SPDK_SOCK_PROGRESS_US);
        ctx->progress_timer_armed = 1;
    }
    ...
    struct evpl_spdk_thread *ctx)
{
    ...
    if (ctx->poll) {
        spdk_poller_unregister(&ctx->poll);
    }
    ...
    struct evpl_bind *listen_bind)
{
    ...
    if (!s->sock) {
        evpl_spdk_sock_error("Failed to listen on %s:%u: %s", ip, port,
                             strerror(errno));
        return -1;
    }
    ...
    if (rc) {
        evpl_spdk_sock_error("spdk_sock_group_add_sock failed: %s", strerror(errno));

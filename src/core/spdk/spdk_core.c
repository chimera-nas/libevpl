 ...
 * otherwise it is armed to the nearest evpl timer deadline or disarmed.
 * Native wait_ms does not impose periodic idle wakeups on the host.
 */
static void
evpl_core_spdk_rearm(struct evpl_core *evc)
{
    ...
    pending = evpl->poll_mode ||
        ...
        s->last_wait_full || s->wait_pending;
...
evpl_core_spdk_pump(struct evpl_core *evc)
{
    ...
    /* Service generic providers once per host timeslice; native fast-path
     * poll iterations are disabled below so fd/timer sweeps still run. */
    uint64_t activity = evpl->activity;
    if (evpl->poll_mode) {
        for (int i = 0; i < evpl->num_poll; i++) {
            struct evpl_poll *poll = &evpl->poll[i];
            if (poll->callback) {
    ...
    int               max_events)
{
    ...
    /* One bounded dispatcher pass per host timeslice. Native spin grace and
     * skipped fd/timer sweeps do not apply to a host-owned scheduler. */
    evpl->config.poll_iterations = 0;
    evpl->spin_ticks             = 0;
    ...
    int               max_msecs)
{
    ...
    /* The provider prepare callbacks may veto sleep even when no fd is
     * ready (e.g. libfabric FI_WAIT_NONE). Preserve that request for the
     * post-pump interrupt rearm, while never blocking the host reactor. */
    s->wait_pending = (max_msecs == 0);
...
/* Runs outside the evpl pump; the host remains responsible for all progress. */
static int
evpl_spdk_destroy_poll(void *arg)
{
    struct evpl           *evpl = arg;
    struct evpl_core_spdk *s    = &evpl->core.u.spdk;
    evpl_completion_t      callback;
    void                  *private_data;

    if (evpl_has_pending_binds(evpl) || evpl->block_pending ||
        evpl->num_active_deferrals) {
        return SPDK_POLLER_IDLE;
    }
    callback     = s->destroy_callback;
    private_data = s->destroy_private;
    spdk_poller_unregister((struct spdk_poller **) &s->destroy_poller);
    evpl_destroy(evpl);
    if (callback) {
        callback(private_data);
    }
    return SPDK_POLLER_BUSY;
} /* evpl_spdk_destroy_poll */

void
evpl_spdk_destroy_async(
    struct evpl      *evpl,
    evpl_completion_t callback,
    void             *private_data)
{
    ...
    evpl_core_abort_if(spdk_get_thread() != s->thread || evpl->destroying,
                       "destroy must be requested once on the owning SPDK thread");
    evpl->destroying = 1;
    ...
    s->destroy_private  = private_data;
    s->destroy_poller   = spdk_poller_register(evpl_spdk_destroy_poll, evpl, 0);
    evpl_core_abort_if(!s->destroy_poller, "destroy poller registration failed");
    evpl_kick(evpl);
} /* evpl_spdk_destroy_async */

...
/*
 * spdk_sock TCP ping-pong between evpl threads: covers the listener-thread
...
static struct evpl_thread_config *
worker_config(void)
{
    struct evpl_thread_config *config = evpl_thread_config_init();

    evpl_thread_config_set_poll_mode(config, getenv("EVPL_TEST_NO_POLL") ? 0 : 1);
    return config;
} /* worker_config */
    ...
    char *argv[])
{
    ...
    listener = evpl_listener_create_config(worker_config());

    /* Blocks until server_init attached the binding on the worker. */
    server_thread = evpl_thread_create(worker_config(), server_init, server_shutdown,
                                       &state);
    ...
    client_thread = evpl_thread_create(worker_config(), client_init, NULL, &state);

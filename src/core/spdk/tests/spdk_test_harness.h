...
static int
evpl_spdk_test_thread_op(
    struct spdk_thread *thread,
    enum spdk_thread_op op)
{
    return op == SPDK_THREAD_OP_NEW ? evpl_spdk_test_new_thread(thread) : 0;
} // evpl_spdk_test_thread_op

static bool
evpl_spdk_test_thread_op_supported(enum spdk_thread_op op)
{
    return op == SPDK_THREAD_OP_NEW || op == SPDK_THREAD_OP_RESCHED;
} // evpl_spdk_test_thread_op_supported
...
evpl_spdk_test_init(int nreactors)
{
    ...
    rc = spdk_thread_lib_init_ext(evpl_spdk_test_thread_op,
                                  evpl_spdk_test_thread_op_supported,

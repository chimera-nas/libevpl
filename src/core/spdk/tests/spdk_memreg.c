 ...
 * with spdk_mem_register as it is built, and slabs are 4 KiB aligned.
...
static atomic_int covered;
static atomic_int covered_aligned;
    ...
    size_t                          size)
{
     ...
     * registers its own EAL heap regions too, and those need not be 4 KiB
    ...
    char *argv[])
{
    ...
    evpl_test_abort_if(!covered_aligned,
                       "the slab's registered region was not 4 KiB aligned");

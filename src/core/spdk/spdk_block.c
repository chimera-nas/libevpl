 ...
 * evpl_activity() and wakes an idle guest after callbacks, so a
...
struct evpl_spdk_bdev_device {
    ...
    uint32_t                  block_size;
    atomic_uint_fast64_t      num_blocks;
    size_t                    buf_align;
...
struct evpl_spdk_bdev_request {
    ...
    int                            niov;
    struct iovec                   submit_iov[EVPL_SPDK_BDEV_MAX_IOV];
    int                            submit_niov;
 ...
 * activity for busy accounting, and wake an idle guest after callbacks so
 * queued work cannot be stranded in interrupt mode.
 */
static void
    ...
    int                            status)
{
    ...
    /* The core may close the queue and device from the final callback. */
    queue->outstanding--;
        ...
        switch (request->kind) {
            case EVPL_BLOCK_OP_READ:
                rc = spdk_bdev_readv(dev->desc, queue->ch,
                                     request->submit_iov, request->submit_niov,
...
/*
 * Validate and stage a data op. Returns zero when ready to submit, or -1
 * after completing the request with an error.
 */
static int
    ...
    int                            is_write)
{
    ...
    int                           misaligned = 0;
    int                           i;
    uint64_t                      size = atomic_load(&dev->num_blocks) * (uint64_t) dev->block_size;

    if (niov <= 0 || niov > EVPL_SPDK_BDEV_MAX_IOV) {
    ...
    if (length == 0 || length > evpl_shared->config->buffer_size ||
        (offset % dev->block_size) ||
        (length % dev->block_size) ||
    ...
    if (!misaligned && niov <= EVPL_SPDK_BDEV_MAX_IOV) {
        ...
        request->niov = niov;
        memcpy(request->submit_iov, request->iov, niov * sizeof(struct iovec));
        request->submit_niov = niov;
        return 0;
    }
    ...
    request->submit_iov[0].iov_base = request->bounce.data;
    request->submit_iov[0].iov_len  = length;
    request->submit_niov            = 1;
    ...
    void                    *private_data)
{
    ...
    if (sync && dev->has_write_cache) {
        if (!dev->supports_flush) {
            evpl_spdk_bdev_request_complete(request, ENOTSUP);
            return;
        }
    ...
    void                    *private_data)
{
    ...
    if (!queue->dev->supports_flush) {
        evpl_spdk_bdev_request_complete(request,
                                        queue->dev->has_write_cache ? ENOTSUP : 0);
        return;
    }
...
evpl_spdk_bdev_write_zeroes(
    ...
    void                    *private_data)
{
    /* A private sentinel distinguishes write_zeroes in the submit switch;
     * the core accounts it as EVPL_BLOCK_OP_WRITE. */
    ...
    struct evpl_block_queue *bq)
{
    ...
    evpl_spdk_bdev_abort_if(queue->outstanding,
                            "core must drain I/O before closing a queue");
    ...
    void                     *event_ctx)
{
    ...
    if (dev->bdev_pub->event_callback &&
        (type == SPDK_BDEV_EVENT_REMOVE || type == SPDK_BDEV_EVENT_RESIZE)) {
        dev->bdev_pub->event_callback(dev->owner, dev->bdev_pub,
                                      type == SPDK_BDEV_EVENT_REMOVE ? EVPL_BLOCK_EVENT_REMOVE : EVPL_BLOCK_EVENT_RESIZE
                                      ,
                                      dev->bdev_pub->event_private);
    }
} /* evpl_spdk_bdev_event_cb */
    ...
    void                        *ctx)
{
    ...
    if (!dev->block_size || dev->num_blocks > UINT64_MAX / dev->block_size ||
        (dev->buf_align & (dev->buf_align - 1)) ||
        dev->buf_align > evpl_shared->config->buffer_size ||
        spdk_bdev_get_md_size(dev->bdev) != 0) {
        spdk_bdev_close(dev->desc);
        evpl_free(dev);
        evpl_free(bdev_pub);
        complete(evpl, NULL, ENOTSUP, ctx);
    ...
    bdev_pub->max_request_size = evpl_shared->config->buffer_size;

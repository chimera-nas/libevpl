    ...
    int              is_listen)
{
    ...
    t->state        = is_listen ? EVPL_TLS_STATE_CONNECTED : EVPL_TLS_STATE_HANDSHAKING;
    t->ssl          = evpl_tls_session_create(evpl, is_server);
    t->is_attached  = 0;
    t->ktls_checked = 0;
    t->alpn_len     = 0;
...
static void
evpl_tls_attach_discard(
    struct evpl *evpl,
    void        *accepted)
{
    struct evpl_accepted_tls *a = accepted;
    ...
    struct evpl_bind *bind)
{
    ...
    if (t->recv1.length) {
        evpl_iovec_release_internal(evpl, &t->recv1);
        t->recv1.length = 0;
    }

    if (t->recv2.length) {
        evpl_iovec_release_internal(evpl, &t->recv2);
        t->recv2.length = 0;
    }
    ...
    while (t->free_datagrams) {
        datagram = t->free_datagrams;
        LL_DELETE(t->free_datagrams, datagram);
        evpl_iovec_release_internal(evpl, &datagram->iovec);
        evpl_free(datagram);
    }
} /* evpl_tls_close */
...
struct evpl_protocol evpl_socket_tls = {
    ...
    .discard_accepted = evpl_tls_attach_discard,

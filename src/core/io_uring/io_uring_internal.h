...
/* How many zero-copy sends to keep outstanding on one stream.  The batch is
 * chained, so this is how much DMA/notification latency can be hidden before
 * the pipeline has to drain and refill. */
#define EVPL_IO_URING_SEND_DEPTH        64

/* Smallest send that is worth doing zero-copy.  Below this the page pinning
 * and the extra notification completion cost more than the copy they avoid. */
#define EVPL_IO_URING_SEND_ZC_THRESHOLD 16384
...
/* Process-wide registry of allocator slabs eligible for FIXED_BUF
 * registration.  Only the index assignment is shared; each ring registers the
 * slabs it touches into its own buffer table. */
struct evpl_io_uring_shared {
    evpl_mutex_t                  buf_lock;
...
struct evpl_io_uring_request {
    ...
    union {
        struct {
            ...
            /* A zero-copy send keeps its own references on the payload, so the
            * send queue can advance as soon as the socket accepts the data
            * while the buffers live on until the kernel stops reading them. */
...
#define EVPL_IO_URING_ZCRX_MAX_IFQ 16
...
struct evpl_io_uring_zcrx_state {
    /* Receive buffer size the kernel settled on, and whether the area is
     * huge-page backed (which is what allows it to exceed one page). */
    uint32_t                        rx_buf_len;
    unsigned                        area_huge : 1;

    /* Diagnostics: how much payload each completion actually carried. */
    uint64_t                        stat_frags;
    uint64_t                        stat_bytes;
    ...
    unsigned int                    rq_entries_wanted; /* one per buffer in the area */
    unsigned int                    rq_mask;
    uint32_t                        zcrx_id;
    /* The hardware receive queue this ifq is bound to and the NAPI instance
     * that services it; an accepted socket reports the latter through
     * SO_INCOMING_NAPI_ID, which is how it finds its ifq. */
    unsigned int                    rxq;
    uint32_t                        napi_id;
...
struct evpl_io_uring_context {
    ...
    /* ZCRX state, one entry per registered ifq (= per hardware receive
     * queue), valid only when effective.zcrx == 1.  A single application
     * thread can drain several queues this way, each queue's driver and
     * TCP work landing on its own softirq core. */
    struct evpl_io_uring_zcrx_state *zcrx[EVPL_IO_URING_ZCRX_MAX_IFQ];
    int                              num_zcrx;
    /* Sockets whose NAPI id matched none of our queues and were put on the
     * first ifq, where the kernel copies rather than places their data. */
    uint64_t                         stat_zcrx_unmatched;

    /* Diagnostics: how many zero-copy sends the kernel actually sent from our
     * pages, versus how many it fell back to copying, and how much each send
     * managed to carry. */
    uint64_t                         stat_zc_notifs;
    uint64_t                         stat_zc_copied;
    uint64_t                         stat_zc_sends;
    uint64_t                         stat_zc_iovs;
    uint64_t                         stat_zc_bytes;
};
...
evpl_io_uring_request_free(
    ...
    struct evpl_io_uring_request *req)
{
    /* Guard against a double free: the request must neither be pushed onto
     * the freelist twice nor end its owner's operation twice. */
    if (req->on_freelist) {
        return;
    }
    req->on_freelist = 1;

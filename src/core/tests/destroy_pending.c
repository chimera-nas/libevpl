...
static void notify(struct evpl *evpl, struct evpl_bind *bind,
                   struct evpl_notify *event, void *private_data)
{
    unsigned int *disconnected = private_data;
    (void) evpl;
    (void) bind;
    if (event->notify_type == EVPL_NOTIFY_DISCONNECTED) (*disconnected)++;
}
int main(void)
{
    ...
    if (!evpl_bind(evpl, EVPL_DATAGRAM_SOCKET_UDP, endpoint, notify, &disconnected)) return 1;

...
static void
evpl_thread_spdk_finished(void *ctx)
{
    struct evpl_thread *thread = ctx;
    int                 detached;
    ...
    callback     = thread->completion;
    private_data = thread->completion_private;
    pthread_cond_signal(&thread->cond);
    pthread_mutex_unlock(&thread->lock);

    if (detached) {
        evpl_wakeup_close(&thread->stop_wakeup);
...
evpl_thread_spdk_teardown(void *ctx)
{
    ...
    evpl_remove_event(evpl, &thread->stop_event);
    spdk_poller_unregister((struct spdk_poller **) &thread->spdk_teardown_poller);
    if (thread->shutdown_callback) {
        thread->shutdown_callback(evpl, thread->private_data);
    }
    evpl_destroy_async(evpl, evpl_thread_spdk_finished, thread);
    ...
    int                             wait_ready)
{
    ...
    evpl_thread->config = config; /* ownership transferred, as for native workers */
...
evpl_thread_destroy_spdk(struct evpl_thread *evpl_thread)
{
    ...
    if (free_now) {
        ...
        pthread_cond_destroy(&evpl_thread->cond);
        pthread_mutex_destroy(&evpl_thread->lock);
        evpl_free(evpl_thread);
    }
} /* evpl_thread_destroy_spdk */

void
evpl_thread_destroy_async_spdk(
    ...
    void               *private_data)
{
    ...
    thread->completion_private = private_data;
    thread->detached           = 1;
    /* Publish the completion before waking the worker. */
    evpl_thread_abort_if(evpl_wakeup_signal(&thread->stop_wakeup) < 0,
                         "thread stop signal failed");
    pthread_mutex_unlock(&thread->lock);
} /* evpl_thread_destroy_async_spdk */
